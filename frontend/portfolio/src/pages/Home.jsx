...
//             <a
//               href={profile?.resume ? profile.resume : "/AnjaleeBisen.pdf"}
//               target="_blank"
...
//           <img
//             src={profile?.photo ? profile.photo : "/anjaleephoto.jpeg"}
//             alt="Anjalee Bisen"
...
//               <button onClick={() => setShowResumeModal(false)} className="bg-gray-700 px-4 py-1 rounded-lg">Cancel</button>
//               <button
//                 onClick={async () => {
...
//             />
//             <div className="flex justify-end gap-3">
//               <button onClick={() => setShowPhotoModal(false)} className="bg-gray-700 px-4 py-1 rounded-lg">Cancel</button>
//               <button
//                 onClick={async () => {
//                   const token = localStorage.getItem("token");
//                   const formData = new FormData();
...
export default function Home() {
  ...
  // Fetch profile data with cache and instant update
  const fetchProfile = useCallback(async () => {
    try {
      // Load cache first
      const cached = getCache("profileCache");
      if (cached) setProfile(cached);

      // Fetch fresh data
      const res = await axios.get(`${API}/api/profile`);
      setProfile(res.data);
      setCache("profileCache", res.data);
    } catch (err) {
  ...
  const handleResumeUpdate = async () => {
    if (!newResume) return alert("Select a file first");
    try {
      const token = localStorage.getItem("token");
      const formData = new FormData();
      formData.append("resume", newResume);
    ...
    >
      ...
      <div className="max-w-7xl w-full grid md:grid-cols-2 gap-12 items-center relative z-10">
        ...
        <div className="text-white space-y-6 text-center md:text-left">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
            <a
              href={profile?.resume || "/AnjaleeBisen.pdf"}
              target="_blank"
              rel="noopener noreferrer"
              className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl text-white font-medium shadow-lg hover:scale-105 transition duration-300"
            >
        ...
        <div className="flex flex-col items-center relative">
          <div className="absolute w-72 h-72 bg-cyan-500 rounded-full blur-3xl opacity-20"></div>
          <img
            src={profile?.photo || "/anjaleephoto.jpeg"}
            alt="Anjalee Bisen"
            className="w-56 h-56 sm:w-64 sm:h-64 lg:w-72 lg:h-72 object-cover rounded-full border-4 border-cyan-400 shadow-[0_0_40px_rgba(34,211,238,0.4)] hover:scale-105 transition duration-500 relative z-10"
          />
          {admin && (
      ...
      {showResumeModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-gray-900 border border-white/10 p-6 rounded-2xl w-[90%] sm:w-96 space-y-4 shadow-2xl">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowResumeModal(false)} className="bg-gray-700 px-4 py-1 rounded-lg">Cancel</button>
              <button onClick={handleResumeUpdate} className="bg-cyan-500 px-4 py-1 rounded-lg text-black font-medium">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showPhotoModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-gray-900 border border-white/10 p-6 rounded-2xl w-[90%] sm:w-96 space-y-4 shadow-2xl">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowPhotoModal(false)} className="bg-gray-700 px-4 py-1 rounded-lg">Cancel</button>
              <button onClick={handlePhotoUpdate} className="bg-cyan-500 px-4 py-1 rounded-lg text-black font-medium">Save</button>
            </div>
          </div>
        </div>
      )}
