...
export default function Skills() {
  ...
  // Fetch skills (cache first, then fresh)
  useEffect(() => {
    const fetchSkills = async () => {
      try {
        const cached = getCache("skillsCache");
        if (cached && Array.isArray(cached)) setSkillCategories(cached);
  ...
  const updateSkill = async () => {
    if (!selectedId || !title || !skillsInput)
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 sm:p-8 mb-10 max-w-xl mx-auto shadow-xl">
            ...
            <div className="flex justify-center gap-4">
              {mode === "add" && (
                <button
                  onClick={addSkill}
                  className="bg-green-600 px-4 py-2 rounded hover:scale-105 transition"
                >
                  Save
                </button>
              )}
              {mode === "update" && (
                <button
                  onClick={updateSkill}
                  className="bg-yellow-600 px-4 py-2 rounded hover:scale-105 transition"
                >
                  Update
                </button>
              )}
              {mode === "delete" && (
                <button
                  onClick={deleteSkill}
                  className="bg-red-600 px-4 py-2 rounded hover:scale-105 transition"
                >
                  Delete
                </button>
              )}
